...
void run_tests() {
    ...
    struct Result testRes = run_simulation(100000, NULL, 2, 16,
                                           4, 8, 16, 10,
                                           20, 40, 2,
...
int main(int argc, char *argv[]) {
        ...
        switch (opt) {
            ...
            case 's':
                if (string_to_uint32_t(optarg, &cacheLineSize)) {
                    fprintf(
                        stderr,
                        "Invalid value for --cacheline-size. Must be a decimal number between 0 and 4294967295.\n");
                    return 1;
                }
                break;
            case 'u':
                if (string_to_uint32_t(optarg, &numLinesL1)) {
                    fprintf(
                        stderr,
                        "Invalid value for --num-lines-l1. Must be a decimal number between 0 and 4294967295.\n");
                    return 1;
                }
                break;
            case 'v':
                if (string_to_uint32_t(optarg, &numLinesL2)) {
                    fprintf(
                        stderr,
                        "Invalid value for --num-lines-l2. Must be a decimal number between 0 and 4294967295.\n");
                    return 1;
                }
                break;
            case 'w':
                if (string_to_uint32_t(optarg, &numLinesL3)) {
                    fprintf(
                        stderr,
                        "Invalid value for --num-lines-l3. Must be a decimal number between 0 and 4294967295.\n");
                    return 1;
                }
                break;
            case 'x':
                if (string_to_uint32_t(optarg, &latencyCacheL1)) {
                    fprintf(
                        stderr,
                        "Invalid value for --latency-cache-l1. Must be a decimal number between 0 and 4294967295.\n");
                    return 1;
                }
                break;
            case 'y':
                if (string_to_uint32_t(optarg, &latencyCacheL2)) {
                    fprintf(
                        stderr,
                        "Invalid value for --latency-cache-l2. Must be a decimal number between 0 and 4294967295.\n");
                    return 1;
                }
                break;
            case 'z':
                if (string_to_uint32_t(optarg, &latencyCacheL3)) {
                    fprintf(
                        stderr,
                        "Invalid value for --latency-cache-l3. Must be a decimal number between 0 and 4294967295.\n");
                    return 1;
                }
                break;
            case 'm':
                if (string_to_uint8_t(optarg, &mappingStrategy) || mappingStrategy > 2) {
                    fprintf(
                        stderr,
                        "Invalid value for --mapping-strategy. Must be 0 (direct-mapped), 1 (fully associative), or 2 (set-associative).\n");
                    return 1;
                }
                break;
            case 'c':
    ...
    if (mappingStrategy == 2 && (numLinesL1 % NUM_LINES_PER_SET + numLinesL2 % NUM_LINES_PER_SET + numLinesL3 %
                                 NUM_LINES_PER_SET != 0)) {
        fprintf(
            stderr, "Invalid options %s: the number of lines in each level must be a multiple of NUM_LINES_PER_SET\n",
            programName);
        return_code = 1;
        goto cleanup;
    }
    ...
    struct Result simulationRes = run_simulation(cycles, tracefile, numCacheLevels, cacheLineSize,
                                                 numLinesL1, numLinesL2, numLinesL3, latencyCacheL1,
                                                 latencyCacheL2, latencyCacheL3, mappingStrategy,
                                                 numRequests, requests);

    printf("Simulation Results:\n");
    printf("Cycles: %u\n", simulationRes.cycles);
    printf("Hits: %u\n", simulationRes.hits);
    printf("Misses: %u\n", simulationRes.misses);
